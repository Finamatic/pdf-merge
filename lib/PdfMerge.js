...
function assembleExecArgs() {
	...
	var execArgs    = _.chain(this.pdfFiles).clone()
		...
		.value();
	execArgs.push('cat', 'output', this.isWin ? tmpFilePath : shellescape([tmpFilePath]),'verbose');
	return execArgs;
}
...
PDFMerge.prototype.merge = function(callback) {
	...
	if(this.isWin) {
		...
		this.exec('pdftk ' + this.execArgs.join(' '),{timeout:this.timeout}, execCallbackHandler);
	}
